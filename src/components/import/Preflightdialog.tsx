...
const TYPE_LABEL_KEYS: Record<string, string> = {
  category: "import.preflight.type_labels.category",
  client_category: "import.preflight.type_labels.client_category",
  supplier: "import.preflight.type_labels.supplier",
  packaging: "import.preflight.type_labels.packaging",
  product: "import.preflight.type_labels.product",
};

function typeLabel(t: TFunction<"common">, type: string) {
    ...
    <Card className="border-amber-300 dark:border-amber-700">
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base text-amber-700 dark:text-amber-400">
          ...
          {t("import.preflight.title")}
        </CardTitle>
        <p className="text-xs text-muted-foreground">
          {t("import.preflight.description")}
        </p>
      </CardHeader>
      ...
      <CardContent className="space-y-3">
        {hasAutoCreate && (
          <Section
            title={t("import.preflight.auto_create_title")}
              ...
              {(report.auto_creatable ?? []).map((dep, i) => (
                <li key={i} className="space-y-0.5">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    {t(dep.needed_by_rows.length > 1 ? "import.preflight.used_in_lines" : "import.preflight.used_in_line")} {dep.needed_by_rows.slice(0, 5).join(", ")}
        ...
        {hasBlocking && (
          <Section
            title={t("import.preflight.blocking_title")}
            count={report.blocking?.length ?? 0}
            variant="error"
          >
            <ul className="space-y-2">
              {(report.blocking ?? []).map((blocking, i) => (
                <li key={i} className="space-y-0.5">
                  <div className="flex items-center gap-2 text-sm">
                    <Badge variant="outline" className="text-[10px] font-normal">
                      {typeLabel(t, blocking.type)}
                    </Badge>
                    <span className="font-medium">{blocking.name}</span>
                  </div>
                  <p className="text-xs text-destructive/80">{blocking.reason}</p>
                  <p className="text-[11px] text-muted-foreground">
                    {t(blocking.needed_by_rows.length > 1 ? "import.preflight.lines" : "import.preflight.line")} {blocking.needed_by_rows.slice(0, 5).join(", ")}
                    {blocking.needed_by_rows.length > 5 && " +" + (blocking.needed_by_rows.length - 5)}
                  </p>
                </li>
              ))}
            </ul>
        ...
        {hasRowErrors && (
          <Section
            title={t("import.preflight.invalid_values_title")}
            count={report.row_errors?.length ?? 0}
            variant="warning"
          >
            <ul className="space-y-1.5">
              {(report.row_errors ?? []).map((rowError, i) => (
                <li key={i} className="flex items-start gap-2 text-xs">
                  <span className="mt-0.5 shrink-0 rounded bg-amber-100 px-1.5 py-0.5 font-mono text-amber-700 dark:bg-amber-900 dark:text-amber-300">
                    {t("import.preflight.row_short", { row: rowError.row })}
                  </span>
                  <span>
                    <span className="font-medium text-foreground">{rowError.field}</span>
                    {" - "}
        ...
        <div className="flex flex-col gap-2 pt-1 sm:flex-row">
          {canAutoCreate && (
            ...
            >
              ...
              {t("import.preflight.create_continue")}
            </Button>
          )}
          ...
          >
            ...
            {canAutoCreate ? t("import.preflight.cancel_import") : t("import.preflight.close_and_fix")}
          </Button>
        </div>

        {(hasBlocking || hasRowErrors) && (
          <p className="text-center text-xs text-muted-foreground">
            {t("import.preflight.fix_and_retry")}
          </p>
        )}
      </CardContent>
    </Card>

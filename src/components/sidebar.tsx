...
import { useMenuItems } from "@/utils/constants";
    ...
    <div className="relative">
      <button
        ...
        className="relative inline-flex items-center justify-center
                      ...
                      hover:text-gray-600 focus:outline-none focus:ring-2
                       focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-200 dark:text-muted-foreground dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-offset-gray-800"
      ...
      {open && (
        <div
          ...
          className={cn(
            "bg-background/50 backdrop-blur-md z-[9999] w-screen fixed top-0 left-0 bottom-0 right-0 h-screen"
          )}
        ></div>
      )}
      <div
        className={cn(
          "h-screen overflow-auto scrollbar absolute top-0 z-[999999] left-0 bottom-0 py-5 space-y-5 flex flex-col items-center justify-start -translate-y-3  bg-card w-[370px] transition-transform duration-300 ease-in-out",
            ...
            return (
              <li className="px-1 pl-4" key={index}>
                <Button
                  ...
                  className={cn(
                    "flex rounded justify-between items-center text-sm hover:text-indigo-700 hover:bg-indigo-300 dark:hover:bg-gray-700 dark:hover:text-white font-medium w-full",
                    basePath.toLowerCase() === item.name.toLowerCase() &&
                    "text-indigo-700 bg-indigo-100 dark:bg-gray-700 dark:text-white"
                  )}
                >
                  <div className="flex gap-2 items-center capitalize justify-between">
                    {Icon && <Icon className={"w-3 h-3"} />}
                    {t(`navigation.groups.${item.name}`)}
                  </div>
                  {item.menu && (
                    <ChevronDown
                      className={cn(
                  ...
                  {item.menu?.map((el, index) => (
                    <li key={index} className="w-full ">
                      <a
                        href={el.link}
                        className={cn(
                          "text-sm p-2 w-full block transition font-medium text-muted-foreground hover:bg-indigo-50 hover:text-indigo-700 dark:hover:bg-gray-700 dark:hover:text-white rounded"
                        )}
                      >
                        {el.text}
                      </a>

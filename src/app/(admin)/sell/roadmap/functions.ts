...
const groupByCustomer = (bills: Bill[]) => {
  ...
  bills.forEach((bill: Bill) => {
    ...
    const customerName = bill?.customer_details?.name ?? i18n.t("newsell.default_customer");
...
const exportToExcel = (groupedData: GroupedData, fileName: string) => {
  ...
  Object.entries(groupedData).forEach(([customerId, customerData]) => {
    ...
    rows.push({
      [i18n.t("roadmap.excel.customer")]: customerData.customer_name,
      [i18n.t("roadmap.excel.total_amount")]: customerData.total_amount,
      [i18n.t("roadmap.excel.total_packages")]: customerData.total_package,
      [i18n.t("roadmap.excel.total_packages_recorded")]: customerData.total_package_recorded,
    });
    ...
    Object.entries(customerData.total_products).forEach(
      ([productId, productData]) => {
        rows.push({
          [i18n.t("roadmap.excel.product")]: productData.product_name,
          [i18n.t("roadmap.excel.total_quantity")]: productData.total_quantity,
        });
      }
    );
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, i18n.t("roadmap.excel.sheet_name"));

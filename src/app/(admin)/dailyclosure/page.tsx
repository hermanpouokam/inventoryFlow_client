...
export default function Page() {
    ...
    const sides = React.useMemo(() => {
        ...
        return [
            {
                nameKey: "daily_closure.cash_balance",
                data: () =>
                    formatteCurrency(
            ...
            {
                nameKey: "daily_closure.daily_sales",
                data: () => {
                ...
                subText: () => {
                    ...
                    return previousMonthSales === 0
                        ? "N/A"
                        : t("daily_closure.compared_to_previous_period", { percent: percentageChange.toFixed(2) });
                },
                subTextColor: () => {
                    const currentMonthSales = bills
                        .filter(
            ...
            {
                nameKey: "daily_closure.daily_profit",
                data: () => {
                    const currentMonthSales = bills.filter(
                        (bill) =>
                            bill.state !== "created" &&
                ...
                subText: () => {
                    ...
                    return previousMonthSales === 0
                        ? "N/A"
                        : t("daily_closure.profit_change", { percent: percentageChange.toFixed(2), direction: t(percentageChange >= 0 ? "daily_closure.more" : "daily_closure.less") });
                },
                subTextColor: () => {
                    const currentMonthSales = bills
                        .filter(
        ...
        }, [bills, salespoint, salespointError, salespointStatus, t])
    ...
    return (
        <main className="space-y-4">
            ...
            <CardBodyContent className="space-y-3">
                <h4 className="font-semibold text-base ">{t("daily_closure.title")}</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4">
                    <Popover>
                        <PopoverTrigger asChild>
                            ...
                            >
                                ...
                                {date ? moment(date).format('DD-MM-YYYY') : <span>{t("daily_closure.select_date")}</span>}
                            </Button>
                        </PopoverTrigger>
                    ...
                    <Popover>
                        ...
                        <PopoverContent className="p-0">
                            <Command>
                                <CommandInput placeholder={t("search.by_name")} />
                                <CommandList>
                                    <CommandEmpty>{t("empty.no_items_found")}</CommandEmpty>
                                    <CommandGroup>
                                        {salespoint.map((sp) => (
                                            <CommandItem
                                                value={`${sp.name}`}
                ...
                {sides.map((side, i) => (
                    ...
                    >
                        <div className="flex flex-row justify-between items-center">
                            <p className="text-sm font-semibold">{t(side.nameKey)}</p>

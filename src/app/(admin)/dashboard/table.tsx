...
const BillTable: React.FC<BillTabe> = ({ data }) => {
  ...
  const columns: ColumnDef<Bill>[] = [
    ...
    },
    {
      ...
      header: () => (
        <div className="text-center w-[140px]">{t("bills.columns.bill_number")}</div>
      ),
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: () => <div className="text-center w-[220px]">{t("bills.columns.sales_point")}</div>,
      cell: ({ row }) => (
      ...
      header: ({ column }) => {
        return (
          <div
            ...
            onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}
          >
            <span>{t("bills.columns.status")}</span>
            {/* <ArrowUpDown className="ml-2 h-4 w-4" /> */}
          </div>
        );
      },
    ...
    },
    {
      ...
      header: () => <div className="text-center w-[240px]">{t("bills.columns.updated_at")}</div>,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: () => <div className="text-center w-[240px]">{t("bills.columns.action")}</div>,
      cell: ({ row }) => {
        const bill = row.original;
        return (
...

    {
      id: "actions",
      enableHiding: false,
      header: () => <div className="text-left w-[50px]">{t("bills.columns.details")}</div>,
      cell: ({ row }) => {
        const bill = row.original;
        return (

...
export default function Page() {
    ...
    const fields: Field[] = React.useMemo(
        () => [
            ...
            },
            {
                name: "role",
                required: true,
                label: tCommon("users.role"),
                type: "text",
            },
        ],
        [status, tCommon, isAdmin, data]
    );
    ...
    const onSubmit = async () => {
        ...
        try {
            ...
            setFieldError(
                "form",
                tCommon(`${error.response?.data.code.replace(" ", "_")}`)
            );
        } finally {
            setLoading(false);
            // setSteps('')
                    ...
                    <form method='post' className='space-y-5' onSubmit={(e) => handleSubmit(e, onSubmit)}>
                        <CardBodyContent>
                            <div className="flex flex-row justify-between items-center">
                                <h2 className="text-base font-medium">{tCommon("users.actions.add_user")}</h2>
                                <Button
                                    variant="default"
                                    type='submit'
                                    className="bg-indigo-600 hover:bg-indigo-500"
                                >
                                    {tCommon("users.actions.add_user_submit")}
                                </Button>
                            </div>
                        </CardBodyContent>
                        <CardBodyContent className='space-y-5'>
                            ...
                            <Divider className='uppercase my-5'>
                                {tCommon("users.user_information")}
                            </Divider>
                            ...
                            <Divider className='uppercase my-5 space-x-1'>
                                {tCommon("users.user_permissions")}
                                <MuiButton
                                    id="basic-button"
                                    ...
                                    onClick={handleClick}
                                >
                                    {selectedRole ? selectedRole.name : tCommon("all")}

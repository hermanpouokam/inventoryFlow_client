...
export default function Profile() {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            toast({
                title: "Succès",
                description: error.response?.data?.error ? tCommon(`${error.response?.data?.error}`) : tCommon(``),
                variant: "success",
                className: "bg-green-600 border-green-600 text-white",
                icon: <Check className="mr-2" />,
            })
                ...
                <div className="relative w-24 h-24 rounded-full ">
                    {
                        preview ?
                            <Tooltip title={capitalizeFirstLetter(tWords('cancel'))} placement="right-start">
                                <Button onClick={() => { setPreview(null); setImage(null) }} className="absolute right-0 -top-2 w-4 h-8 rounded-full" variant={'destructive'}>
